...
# 因为 bert 的 tokenizer 会自动添加 [CLS]，[SEP] 等 token，
# 所以把这两个统一映射为 74 （之前的 label_vocab 只有 0:73 ）
...
#加载预训练模型
pretrained = AutoModelForTokenClassification.from_pretrained(checkpoint,
...
class Model(torch.nn.Module):
    ...
    def forward(self, inputs, labels=None):
        ...
        if self.tuneing:
            logits =  self.pretrained(**inputs).logits
        else:
            with torch.no_grad():
                logits = pretrained(**inputs).logits
...
#######
def train(epochs):
    lr = 2e-5 if model.tuneing else 2e-3
    ...
    for epoch in range(epochs):
        ...
        if dev_acc > best_result['dev_acc']:
            ...
            }, open('./output/model/bert_crf_model.bin', 'wb'))
